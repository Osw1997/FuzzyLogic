...
for i = 1:length(x)
 for j = 1:length(x)
 D(i) = D(i) + exp(-alpha*( ( x(i)-x(j) )^2 + ( y(i)-y(j) )^2 + ( z(i)-z(j) )^2 ));
 end
end
% % Obtener X_i* y Pi* *************************************************
Pi(k) = max(D(:,k));
...
while (1)
 ...
 for i = 1:length(x)
    D(i,k+1) = D(i,k) - Pi(k)*(exp(-beta*( ( x(i)-x(Xi(k)) )^2 + ( y(i)-y(Xi(k)) )^2 + ( z(i)-z(Xi(k)) )^2 )));
 end
 % % Obtener X_i* y Pi* *************************************************
 k = k + 1;
 repetir = 0;
...
plot3(centroX,centroY,centroZ,'*k','linewidth',2)
title('Tercer caso: sustractivo.')
xlabel(' X')
ylabel(' Y')
zlabel(' Z')